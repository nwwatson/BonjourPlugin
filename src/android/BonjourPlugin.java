...
public class BonjourPlugin extends CordovaPlugin {
	...
	public void initialize(CordovaInterface cordova, CordovaWebView webView) {
		...
		lock = wifi.createMulticastLock("BonjourPluginLock");
		lock.setReferenceCounted(true);
		lock.acquire();

		Log.v("BonjourPlugin", "Initialized");
	}
	...
	public boolean execute(String action, JSONArray args,
		...
		if (action.equals("startServiceDiscovery")) {
			final String type = args.optString(0) + "." + args.optString(1) + ".";
			if (type != null) {
		...
		} else if (action.equals("stopServiceDiscovery")) {
			if (jmdns != null) {
				try {
					jmdns.close();
				} catch (IOException e) {
			...
			Log.e("BonjourPlugin", "Invalid action: " + action);
	...
	private void watch(String type) {
		...
		Log.d("BonjourPlugin", "Watch " + type);
		Log.d("BonjourPlugin",
    ...
    private void list(String type, int timeout) {
        try {
            JmDNS mdnsQuery = JmDNS.create(BonjourPlugin.getIPAddress());
	...
	private void setupWatcher() {
		Log.d("BonjourPlugin", "Setup watcher");
		try {
			jmdns = JmDNS.create(BonjourPlugin.getIPAddress());
			listener = new ServiceListener() {

				public void serviceResolved(ServiceEvent ev) {
					Log.d("BonjourPlugin", "Resolved");

					sendCallback("added", ev.getInfo());
				}

				public void serviceRemoved(ServiceEvent ev) {
					Log.d("BonjourPlugin", "Removed");

					sendCallback("removed", ev.getInfo());
				}

				public void serviceAdded(ServiceEvent event) {
					Log.d("BonjourPlugin", "Added");
    ...
    public void sendListCallback(String action, ServiceInfo[] services) {
        ...
        try {
            ...
            Log.d("BonjourPlugin", "Sending result: " + status.toString());
